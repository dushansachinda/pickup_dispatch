...
public class CommonUtils {
    ...
    public static boolean logout(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        final Optional<Cookie> commonAuthIdCookie = getCommonAuthId(request);
        ...
        if (commonAuthIdCookie.isPresent()) {
            //TOKEN_STORE.remove(appIdCookie.get().getValue());
            //appIdCookie.get().setMaxAge(0);
            //commonAuthIdCookie.get().setMaxAge(0);
            //response.addCookie(appIdCookie.get());
        	Cookie cookie = new Cookie("commonAuthId", null); // Not necessary, but saves bandwidth.
        	cookie.setPath("/");
        	//cookie.set
    ...
    public static Optional<Cookie> getCommonAuthId(final HttpServletRequest request) {
